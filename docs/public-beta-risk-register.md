...
| Parked in-band scope creep | Remote desktop/session work may be mistaken for active AgenticKVM scope. | Provider taxonomy and parking-lot docs state AgenticKVM is out-of-band only. | Parked outside roadmap. | Product/security review before any separate project. | No if docs remain clear. |
